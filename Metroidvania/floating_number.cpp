...
namespace {
	...
	const units::MS kDamageTime = 1750;
}
...
void FloatingNumber::draw(Graphics& graphics) {
	...
	if (type_ == DAMAGE) {
		NumberSprite::DamageNumber(graphics, value_).drawCentered(graphics, center_x_, center_y_ + offset_y_);
	}
	else {
		NumberSprite::ExperienceNumber(graphics, value_).drawCentered(graphics, center_x_, center_y_ + offset_y_);
	}
}